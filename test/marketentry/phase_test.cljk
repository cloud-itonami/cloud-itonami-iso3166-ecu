...
(deftest gate-escalates-non-auto-draft
  (let [r (phase/gate 3 {:op :filing/draft} :commit)]
    (is (= :escalate (:disposition r)))
    (is (= :phase-approval (:reason r)))))
...
(deftest gate-never-relaxes-a-hold
  (doseq [ph [0 1 2 3]]
    (let [r (phase/gate ph {:op :filing/submit} :hold)]
      (is (= :hold (:disposition r)) (str "phase " ph " must not override a governor hold")))))
