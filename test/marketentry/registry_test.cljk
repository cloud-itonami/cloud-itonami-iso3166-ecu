...
(deftest register-draft-and-submit
  (let [d (registry/register-draft "eng-1" "ECU" 0)
        s (registry/register-submit "eng-1" "ECU" 0)]
    (is (= "ECU-DFT-000000" (get d "draft_number")))
    (is (= "ECU-SUB-000000" (get s "submit_number")))
    (is (nil? (get-in d ["certificate" "proof"])))
    (is (= "draft-unsigned" (get-in s ["certificate" "status"])))))

(deftest register-requires-ids
  (is (thrown? Exception (registry/register-draft "" "ECU" 0)))
  (is (thrown? Exception (registry/register-submit "eng-1" "" 0))))

(deftest register-requires-non-negative-sequence
  (is (thrown? Exception (registry/register-draft "eng-1" "ECU" -1)))
  (is (thrown? Exception (registry/register-submit "eng-1" "ECU" -1))))
