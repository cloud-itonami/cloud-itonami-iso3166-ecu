...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Co" :jurisdiction "ECU"
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "ECU" :checklist ["a"] :spec-basis "x"}})
...
(deftest seed-db-covers-all-governor-scenarios
  (testing "the demo engagement set exercises every governor check the test suite depends on"
    (let [mem (store/seed-db)
          dat (store/datomic-seed-db)]
      (doseq [s [mem dat]]
        (is (= 6 (count (store/all-engagements s))))
        (is (false? (:legal-rep-change-approved? (store/engagement s "eng-4")))
            "eng-4 is the legal-rep-change-unapproved fixture")
        (is (false? (:ruc-verified? (store/engagement s "eng-5")))
            "eng-5 is the ruc-unverified fixture")
        (is (not (registry/engagement-fee-matches-claim? (store/engagement s "eng-3")))
            "eng-3 is the fee-mismatch fixture")))))
