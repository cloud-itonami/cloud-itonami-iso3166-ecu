  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Quito Digital S.A."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Quito Digital S.A." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest legal-rep-change-unapproved-is-held-and-unoverridable
  (testing "unapproved SCVS legal-representative change -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:legal-rep-change-unapproved} (-> (store/ledger db) last :basis)))
...
(deftest ruc-unverified-is-held-and-unoverridable
  (testing "unverified RUC when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:ruc-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
...
(deftest filing-draft-and-submit-never-auto-commit
  (testing "regardless of confidence/cleanliness, :filing/draft and :filing/submit always interrupt (never :commit on the FIRST pass)"
    (let [[_db actor] (fresh)
          r1 (exec-op actor "t12" {:op :jurisdiction/assess :subject "eng-6"} operator)]
      (approve! actor "t12")
      (let [rd (exec-op actor "t13" {:op :filing/draft :subject "eng-6"} operator)]
        (is (not= :commit (get-in rd [:state :disposition])) "draft never auto-commits on first pass")))))
