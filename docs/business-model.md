# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Ecuador

## Classification

- Repository: `cloud-itonami-iso3166-ecu`
- ISO 3166: `ECU` (Ecuador)
...
- Social impact: [:ecuadorian-sme-market-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on an Ecuadorian
  public contract
...
- registration walkthrough for SERCOP (Servicio Nacional de
  Contratación Pública), the governing entity of Ecuador's National
  System of Public Procurement, and the RUP (Registro Único de
  Proveedores) single supplier record required to bid
- business/tax registration checklist: a RUC (Registro Único de
  Contribuyentes) tax identifier from the SRI (Servicio de Rentas
  Internas), a precondition for RUP registration on the SERCOP portal
- local-content / preferential-procurement navigation: Ecuadorian
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Ecuadorian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Ecuador).
