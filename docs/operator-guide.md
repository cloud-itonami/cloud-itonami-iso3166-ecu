...
2. Register the client's intake: business type, target public function,
   prior filing history in Ecuador if any.
3. Run the advisor in read-only mode against SERCOP.
4. Compare the checklist against the client's current documentation
   (SRI RUC, RUP registration status).
...
- named referral relationship with Ecuadorian-licensed counsel or a
...
a working referral relationship with Ecuadorian-licensed counsel or a
registered agent for whatever licensed representation the law of
Ecuador requires for actual public-procurement filings.
