(ns marketentry.marketentryllm
  ...
  the ECU public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real SOCE submission. Every
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world SOCE registration
  submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けSOCE提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "legal-rep-change-approved?=" (:legal-rep-change-approved? e)
                        " ruc-verified?=" (:ruc-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-legal-rep-change? e))
                              (:legal-rep-change-approved? e))
                          (or (not (:requires-ruc? e))
                              (:ruc-verified? e)))
                   0.9 0.3)}))
