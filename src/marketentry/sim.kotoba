...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (ECU, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up legal-rep-change-unapproved) ==")
    ...
    (println "== filing/submit eng-4 (legal-rep-change-unapproved -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up ruc-unverified) ==")
    ...
    (println "== filing/submit eng-5 (ruc-unverified -> HARD hold) ==")
